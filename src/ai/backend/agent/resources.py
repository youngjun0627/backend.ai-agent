...
async def detect_resources(k8sCoreApi):
    '''
    ...
    for node in nodes:
        if 'node-role.kubernetes.io/master' in node['metadata']['labels'].keys():
            continue
        node_resources[node['metadata']['name']] = node['status']['capacity']
        slots['cpu'] += int(node['status']['capacity']['cpu'])
        slots['mem'] += int(node['status']['capacity']['memory'][:-2]) * 1024

    known_slot_types.update({ 'cpu': 'count', 'mem': 'bytes' })
