    ...
    async def _rescan_images(self, registry_name: str,
        ...
        async def _scan_tag(sess, rqst_args, image, tag):
            ...
            if self.target_registry['type'] == 'local':
                target_registry_url = \
                    self.target_registry['host']['addr'] + ':' + \
                    str(self.target_registry['host']['port'])
            elif self.target_registry['type'] == 'ecr':
                target_registry_url = \
                    self.target_registry['registry-id'] + '.dkr.ecr.' + \
                    self.target_registry['region'] + '.amazonaws.com'
            ...
            if self.target_registry['type'] == 'ecr':
                ...
                if len(stderr) > 0:
                    if f'The repository with name \'{image}\' does not exist in the registry with id' \
                            not in stderr[0]:
                        cmds = [
                ...
                async with sess.get(hub_url / f'v2/repositories/{username}/',
                    ...
                    if resp.status == 200:
                        ...
                        images.extend(f"{username}/{item['name']}"
                                        ...
                                        if not item['name'].startswith('kernel-') and
                                            (len(image) == 0 or image == item['name']))
                    else:
