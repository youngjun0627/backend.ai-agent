...
class K8sCUDADevice(AbstractComputeDevice):
    pass


class K8sCUDAPlugin(AbstractComputePlugin):
    key = DeviceName('cuda')
    slot_types = (
    ...
    async def list_devices(cls) -> Collection[AbstractComputeDevice]:
        ...
        for node in nodes.items:
            ...
            devices.append(K8sCUDADevice(
                device_id=DeviceId('node' + str(len(devices))),
                hw_location='root',
                numa_node=0,
                memory_size=0,
                processing_units=num_gpus
            ))
        return devices
